...
GeoDash.LineChart.prototype.drawChart = function() {
    ...
    var e = this.formatComma(t.value), o = 55 + this.x(t.date), s = 45 + this.y(t.value);
    d3.select(a).transition().attr("r", this.dotRadius + 3);
    var r = $(".line").get(0).getBBox().width;
    this.x(t.date) >= r && (o -= 55), $(".hoverbox").css("left", o), $(".hoverbox").css("top", s), 
    ...
    s.transition().duration(500).delay(o).attr("d", function(t) {
    ...
    for (var r = 0; r < a.length; r++) {
        var h = a[r].values, n = e.selectAll(".dotset" + r).data(h);
        n.transition().duration(500).delay(o).attr("data", function(t) {
            return t.value;
        }).attr("cx", function(t) {
            return i.x(t[i.options.x]);
        }).attr("cy", function(t) {
            return i.y(t.value);
        }), n.enter().append("circle").attr("class", "dot dotset" + r).attr("r", this.dotRadius).style("fill", function() {
            return i.color(a[r].name);
...
GeoDash.PieChart.prototype.drawChart = function() {
    ...
    this.radius = Math.min(this.width, this.height) / 2, console.log(this.width, this.height), 
    this.color = d3.scale.ordinal().range(this.options.colors), this.arc = d3.svg.arc().outerRadius(this.radius - 10).innerRadius(25), 
    ...
    var a = this.svg.selectAll(".arc").data(this.pie(t)).enter().append("g").attr("class", "arc");
    a.append("path").attr("d", this.arc).style("fill", function(t) {
        return i.color(t.data[i.options.label]);
    }).on("mouseover", function(t) {
        console.log(t), $(i.el).parent().find(".hover-box").html(t.value + "%");
