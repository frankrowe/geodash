...
GeoDash.version = "0.1-dev", GeoDash.noConflict = function() {
    ...
    update: function(t) {
        ...
        t.forEach(function(t) {
            ...
            return t[e];
        }) ]), this.svg.append("g").attr("class", "y axis").call(this.yAxis), this.svg.append("g").attr("class", "bars").selectAll(".bar").data(t).enter().append("rect").attr("class", "bar").attr("x", function(t) {
            return i.x(t[s]);
        ...
        var o = this.svg.append("g").attr("class", "x axis").attr("transform", "translate(0," + this.height + ")"), n = d3.rgb(d3.select(this.el).style("background-color")).toString();
        o.append("rect").attr("width", this.width).attr("height", this.height).attr("transform", "translate(0,1)").style("fill", n), 
        this.options.drawX && o.call(this.xAxis);
    }
    ...
    drawChart: function() {
        ...
        this.y = d3.scale.linear().range([ this.height, 0 ]), this.xAxis = d3.svg.axis().scale(this.x).tickSize(-1 * this.height).tickPadding(10).orient("bottom"), 
        ...
        i.append("g").attr("class", "y axis").call(this.yAxis), t.options.axisLabels && i.select(".y.axis").append("text").attr("class", "axis-label").attr("transform", "rotate(-90)").attr("y", 6).attr("dy", ".71em").style("text-anchor", "end").text(t.options.yAxisLabel), 
    ...
    update: function(t) {
        ...
        var s = this.y.domain();
        this.y.domain([ s[0] - .1 * s[0], s[1] + .1 * s[1] ]);
        ...
        var n = function(t, i) {
            return 10 * i;
        }, a = o.selectAll(".line").data(e);
        a.transition().duration(500).delay(n).attr("d", function(t) {
        ...
        for (var r = 0; r < e.length; r++) {
            var h = e[r].values, l = o.selectAll(".dotset" + r).data(h);
            l.transition().duration(500).delay(n).attr("data", function(t) {
                return t.value;
            }).attr("cx", function(t) {
                return i.x(t[i.options.x]);
            }).attr("cy", function(t) {
                return i.y(t.value);
            }), l.enter().append("circle").attr("class", "dot dotset" + r).attr("r", this.options.dotRadius).style("fill", function() {
                return i.color(e[r].name);
