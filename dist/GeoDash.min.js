...
GeoDash.version = "0.1-dev", GeoDash.noConflict = function() {
    ...
    hoverOnDot: function(t, i, e) {
        ...
        this.x(t.date) >= r && (a -= 55), d3.select(s.el).select(".linehoverbox").style("left", a + "px"), 
        d3.select(s.el).select(".linehoverbox").style("top", n + "px"), d3.select(s.el).select(".linehoverbox").html(o), 
    ...
    update: function(t) {
        ...
        var a = this.y.domain();
        this.y.domain([ a[0] - .1 * a[0], a[1] + .1 * a[1] ]);
        var n = d3.select(this.el + " svg g");
        "auto" == i.options.xInterval ? this.xAxis.ticks(t.length) : this.xAxis.ticks(i.options.xInterval), 
        n.select(".y.axis").transition().call(this.yAxis), n.select(".x.axis").transition().call(this.xAxis);
        var r = function(t, i) {
            return 10 * i;
        }, h = n.selectAll(".line").data(e);
        ...
        for (var s = 0; s < e.length; s++) {
            var o = e[s].values, l = n.selectAll(".dotset" + s).data(o);
