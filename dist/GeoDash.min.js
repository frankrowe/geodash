...
GeoDash.version = "0.1-dev", GeoDash.noConflict = function() {
    ...
    this.el = t, this.options = {}, this.setOptions(i), this.drawChart();
...
GeoDash.Chart.prototype.makeTitle = function() {
    if (console.log(this, this.options), this.options.title) {
        var t = '<div class="title">';
        t += this.options.title, t += "</div>", $(this.el).append(t);
    }
...
F.prototype = GeoDash.Chart.prototype, GeoDash.BarChart.prototype = new F(), GeoDash.BarChart.prototype.constructor = GeoDash.BarChart, 
GeoDash.BarChart.prototype.drawChart = function() {
    this.makeTitle(), this.margin = {
        top: 20,
        right: 10,
        bottom: 20,
        left: 40
    ...
    this.formatPercent = d3.format(".0%"), this.x = d3.scale.ordinal().rangeRoundBands([ 0, this.width ], .05, .5), 
    ...
    var i = this, s = this.options.y, o = this.options.x;
...
F.prototype = GeoDash.Chart.prototype, GeoDash.LineChart.prototype = new F(), GeoDash.LineChart.prototype.constructor = GeoDash.LineChart, 
GeoDash.LineChart.prototype.drawChart = function() {
    ...
    var o = this.formatComma(t.value), a = 55 + this.x(t.date), e = 45 + this.y(t.value);
    d3.select(s).transition().attr("r", this.options.dotRadius + 3);
    var h = $(".line").get(0).getBBox().width;
    this.x(t.date) >= h && (a -= 55), $(".hoverbox").css("left", a), $(".hoverbox").css("top", e), 
    $(".hoverbox").html(o), $(".hoverbox").fadeIn(200);
}, GeoDash.LineChart.prototype.hoverOffDot = function(t, i, s) {
    $(".hoverbox").fadeOut(200), d3.select(s).transition().attr("r", this.options.dotRadius);
    ...
    var o = d3.select(this.el + " svg g");
    this.xAxis.ticks(t.length), o.select(".y.axis").transition().call(this.yAxis), o.select(".x.axis").transition().call(this.xAxis);
    var a = function(t, i) {
        return 10 * i;
    }, e = o.selectAll(".line").data(s);
    e.transition().duration(500).delay(a).attr("d", function(t) {
    ...
    for (var h = 0; h < s.length; h++) {
        var n = s[h].values, r = o.selectAll(".dotset" + h).data(n);
        r.transition().duration(500).delay(a).attr("data", function(t) {
            return t.value;
        }).attr("cx", function(t) {
...
F.prototype = GeoDash.Chart.prototype, GeoDash.PieChart.prototype = new F(), GeoDash.PieChart.prototype.constructor = GeoDash.PieChart, 
GeoDash.PieChart.prototype.drawChart = function() {
    var t = this;
    this.makeTitle(), this.width = "auto" === this.options.width || void 0 === this.options.width ? $(this.el).width() : this.options.width, 
    this.height = "auto" === this.options.height || void 0 === this.options.width ? $(this.el).height() : this.options.height, 
    this.radius = Math.min(this.width, this.height) / 2.2, console.log(this.radius), 
    ...
    s.append("path").attr("d", this.arc).style("fill", function(t) {
        ...
        $(i.el).parent().find(".hover-box").html(t.value + "%"), d3.select(this).style("fill-opacity", 1);
    }).on("mouseout", function() {
        $(i.el).parent().find(".hover-box").html(""), d3.select(this).style("fill-opacity", i.options.opacity);
    });
};