...
GeoDash.version = "0.1-dev", GeoDash.noConflict = function() {
    ...
    drawChart: function() {
        ...
        var i = d3.rgb(d3.select(this.el).style("background")).toString();
    ...
    update: function(t) {
        ...
        this.options.verticalX && (this.xAxisElement.selectAll(".tick").style("display", "none"), 
    ...
    update: function(t) {
        ...
        var l = function(t, i) {
            return 10 * i;
        }, c = h.selectAll(".line").data(e);
        c.transition().duration(500).delay(l).attr("d", function(t) {
        ...
        for (var s = 0; s < e.length; s++) {
            var o = e[s].values, d = h.selectAll(".dotset" + s).data(o);
            d.transition().duration(500).delay(l).attr("data", function(t) {
                return t.value;
            }).attr("cx", function(t) {
                return i.x(t[i.options.x]);
            }).attr("cy", function(t) {
                return i.y(t.value);
            }), d.enter().append("circle").attr("class", "dot dotset" + s).attr("r", this.options.dotRadius).style("fill", function() {
    ...
    drawChart: function() {
        ...
        this.y = d3.scale.ordinal().rangeRoundBands([ 0, this.height ], .05, .1), this.xAxis = d3.svg.axis().scale(this.x).orient("top").tickSize(-1 * this.height, 0, 0).tickFormat(function(i) {
            return t.formatLarge(i);
        }), this.svg = d3.select(this.el).append("svg").attr("width", this.width + this.margin.left + this.margin.right).attr("height", this.height + this.margin.top + this.margin.bottom).attr("class", "barchart-svg").append("g").attr("transform", "translate(" + this.margin.left + "," + this.margin.top + ")"), 
        this.svg.append("g").attr("class", "y axis"), this.xAxisElement = this.svg.append("g").attr("class", "x axis"), 
        this.svg.append("g").attr("class", "bars"), d3.select(t.el).append("div").attr("class", "hoverbox");
    },
    update: function(t) {
        var i = this, e = this.options.y, s = this.options.x;
        t.forEach(function(t) {
            t[e] = +t[e];
        }), this.color = d3.scale.ordinal().range(this.options.barColors), this.x.domain(d3.extent(t, function(t) {
            return t[e];
        })), this.y.domain(t.map(function(t) {
            return t[s];
        }));
        var o = this.svg.select(".bars").selectAll(".bar").data(t);
        o.transition().attr("x", function(t) {
            return i.x(Math.min(0, t[e]));
        }).attr("y", function(t) {
            return i.y(t[s]);
        }).attr("width", function(t) {
            return Math.abs(i.x(t[e]) - i.x(0));
        }).attr("height", i.y.rangeBand()).style("fill", function(t) {
            return i.color(t[s]);
        }).style("fill-opacity", function(t) {
            return t[s] == i.options.highlight ? 1 : i.options.opacity;
        }), o.enter().append("rect").attr("class", "bar").attr("id", function(t) {
            ...
            t[s] == i.options.highlight && (e = 1), d3.select(this).style("fill-opacity", e), 
            d3.select(i.el).select(".hoverbox").transition().style("display", "none");
        }), o.exit().remove(), this.options.drawX ? this.xAxisElement.call(this.xAxis) : this.xAxisElement.selectAll("g").remove();
    },
    setColor: function(t) {
        this.options.barColors = t;
    }
